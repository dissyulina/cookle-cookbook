...
2. See the popular recipes of the site  
   * On the Home Page (index.html), below the hero-image, users can find the eight most popular recipes on the site. These are the recipes that have the most number of likes. 
   * The recipes are organized in a carousel of recipe cards, with an auto-slide feature. The user can also flick the cards on touch screen devices, or click the right arrow (next) or left arrow (previous) to navigate between cards.
   ![Popular Recipes carousel](static/images/readme/testing/user-stories/popular-recipes.jpg "Popular Recipes carousel")   

3. Explore all recipes listed on the site  
   ...
   * Below are the screenshot of the Explore Recipes page.  
...
5. Navigate intuitively and can spot the Sign-Up button right away  
   ...
   * On the landing page, the users can see the Sign-Up and Login button right away, located on the top right of the screen.  
   ![Navbar](static/images/readme/testing/user-stories/navbar-unregistered-user.jpg "Navbar")  
   * The Navbar is disappearing on scroll down but will be re-appearing again on a bit of scroll up. This way the users have easy access to the Navbar, and at the same time, it also saves space, allowing the content to have a full-screen display. 
...
6. Understand what the site is about and how it works  
   * On the Home page, inside the hero-image, a title and tagline for the website are placed to give a user an introduction to the website in a very short statement.  
...
2. See the newest added recipes on the site  
   * On the Home Page, below the popular recipes carousel, users can find the eight newest added recipes in the database.  
   * The recipes are organized in a carousel of recipe cards, with an auto slide feature. The user can also flick the cards on touch screen devices, or click the right arrow (next) or left arrow (previous) to navigate between cards.  
...
4. Be able to Edit or Delete my profile
   ...
   * When the user clicks "Edit Profile", the user will be directed to the Edit Profile page (edit-profile.html), with buttons to Cancel and to Submit the edited profile.
   ![Edit Profile](static/images/readme/testing/user-stories/edit-profile.jpg "Edit Profile")  
   * The user can also delete their profile by clicking the "Delete Profile" button. A modal will pop up to confirm the deletion, as a barrier in case the user accidentally clicked the button. The modal comes with buttons to Cancel and to Delete the profile.  
...
5. Be able to Create a recipe and share it to the site, and Edit or Delete it afterward as needed  
   ...
   * There are various ways to Edit or Delete a recipe: 
     - After the user created a recipe, the user will be directed to the newly created recipe page. On the top right of the recipe page, the user will be able to locate an Edit button and a Delete button.  
     ...
     - The user can also manage all their created recipes from the Profile page by clicking the "Uploaded Recipes" collapse button, and navigating to the ellipsis button to find the option to Edit and Delete.  
   ...
   * Once the user clicked the Delete Recipe button, a modal will pop up to confirm the deletion, as a barrier in case the user accidentally clicked the button. The modal comes with buttons to Cancel and Delete the recipe.  
     ![Modal confirmation before deleting recipe](static/images/readme/testing/user-stories/modal-delete-recipe.jpg "Modal confirmation before deleting recipe")   

6. Be able to Save a recipe that I like to my cookbook, and remove it from my cookbook as needed.  
   * Ways to save a recipe to the user's cookbook:
     ...
     - By clicking the "Save to cookbook" button on a recipe page. When a recipe is already saved to the cookbook, the button will display "Saved to Cookbook" with a checked icon.  
   ...
   * Ways to remove a recipe from a user's cookbook:  
     - By clicking again the Saved to Cookbook button on the Recipe page (see images from the previous point). A modal to confirm the removal will show up.
     - By managing it from the Profile page, where the user can click on the "See All My Saved Recipes" collapse button, then click the trash icon located on the right side of the recipe's name that the user wants to remove.  
     ![Remove recipes from the cookbook on profile page](static/images/readme/testing/user-stories/profile-saved-recipes.jpg "Remove recipes from the cookbook on profile page")  
     - A modal will pop up to confirm the removal, as a barrier in case the user accidentally clicked the button. The modal comes with buttons to Cancel and Remove.  
...
7. Be able to give a like to a recipe, and also unlike it as needed.  
   * To like a recipe, simply click the "Like this recipe" button on the Recipe page. The button will then turn to "Liked", and the number of likes will increase by one. To unlike a recipe, click the "Liked" button again, and it will turn back to "Like this recipe", and the number of likes will decrease by one as well.  
...
8. Be able to write a review on a recipe page, and to edit or delete my review  
   * To write a review, click the "Write A Review" button on a Recipe page. A text-area form will show up with the Cancel and Submit Review buttons.  
   ![Write review](static/images/readme/testing/user-stories/write-review.jpg "Write review")  
   * To edit a review, click on the ellipsis button on the right side of the review that the user wants to edit. It will show the options to edit or delete the review. After choosing the Edit button, a text-area with the review text inside will show up.  
   ...
   * To delete a review, click again on the ellipsis button on the right side of the review that the user wants to delete, and choose the delete option. A modal confirmation will pop up.  
...
3. Be able to edit any recipes listed on the site as needed.  
   * The Edit Recipe button on each of the recipe pages is available for admin to access, even though the recipe was created by another user. This functionality is added for Admin in order to keep the database clean.
  
4. Be able to delete any recipes listed on the site as needed.  
   * The Delete Recipe button on each of the recipe pages is available for admin to access, even though the recipe was created by another user. This functionality is added for Admin in order to keep the database clean.  

5. Be able to delete offensive reviews on the site as needed.  
   * The Delete review button inside the ellipsis dropdown button is available for the admin to access. Admin can only delete a review, not edit it.   
...
[Responsiveness Checker](https://www.websiteplanet.com/webtools/responsive-checker/?url=https%3A%2F%2Fcookle-cookbook.herokuapp.com%2F) was also used to test the responsiveness. Below are a few examples.
...
During the testing, a few errors have been found:   

Error/ Warnings | Actions  
--- | ---   
During the testing to fill out Add Recipe form, I realized that the user can add as many ingredients and directions as they like. It could be until 1000 lines and it will still be allowed. This has made me reconsider adding a limitation to the number of ingredients and directions, in order to prevent them to spoil the page and database. | I ended up setting maximum ingredients and directions to be 30 lines. This was done through the front-end (javascript).  
I could sign up with a username, but then couldn't log in with the same username. It turned out that the Register form and LogIn form didn't have matching minimum / maximum length requirements. | Checked all minimum, maximum, and regex patterns across the forms for user management (Sign Up form, Edit Profile form, Change Password form), to ensure they have the same format requirements.   
An error page after I tried to edit/change the username on Edit Profile form showed up, it couldn't display the profile page. It turned out that the session user was still the old username. | Set a new code in the ```edit_profile``` function, to set the new username as the new session user.    


**For Contact Form (using email js), the test was performed to ensure that:**  
- When a valid Contact form is submitted, a modal will show up for 2 seconds with the text "Message sent" as a response to the user.  
...
All of the above are working properly, no errors were found.  
...
The defensive testing was done to make sure certain actions can only be performed by authorized users/admin. The test was done such as: 
...
- Typing the url to try to access the manage categories page without logging in as Admin : ```https://cookle-cookbook.herokuapp.com/manage_categories``` 
- etc.
When a user tries to perform unauthorized action such as above, a flash message with the red text "You are not authorized to view that page" will be shown.   
...
The duplicate IDs error existed on almost all pages. This happened because of for loop repetition. For example, a review inside a for loop template that has an id attribute. Because the number of the reviews that generated was more than 1, the id repeated and therefore gave an error. The solution for this was to put ```review._id``` inside the id, so that it will become unique.  
All warnings and errors were fixed and I put the Html file back to the test. In the end, no warnings or errors were found on all pages.  
